...
- Client table is unencrypted at rest since it holds PPI mainly NI here...
...
- The (http) status code is not logged in the CLI's console.
...
## Errors
- http response codes are used in context of error situations, ie 404 if a client with the provided `Id` is not found however 400 if `Id` is not in correct format.
- a panic handler may be wrapped around MUX to respond (and log) 5xx to requests that encounter a panic; however, this must not happen in a normal production workflow.

## Database

The file "docker-compose.yml" assumes port 3306 is free and it exposes that port for the MySQL instance in docker. The same holds for port 5432 and Postgres instance.
